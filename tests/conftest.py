...
# from src.database.populate_db import seed_user_groups, seed_movies, seed_users, seed_certifications
...
# # @pytest_asyncio.fixture(scope="session", name="seed_database", autouse=True)
# # async def prepare_database():
# #     """
# #     This runs ONCE per test session:
# #     1. Creates the async engine
# #     2. Creates all tables
# #     3. Seeds initial data (user groups, movies, users)
# #     """
# #     # import src.database
# #     from src.database import Base
...
# #     # Step 2: Seed data using a fresh session
# #     async with AsyncSessionLocal() as session:
# #         async with session.begin():
# #             await seed_user_groups(session)
# #             await seed_movies(num_movies=50)
...
# async def fixture_seed_database(request, session: reset_db):
...
@pytest_asyncio.fixture(name="seed_database", autouse=True)
async def fixture_seed_database(request, reset_db):
    if "no_seed" in request.keywords:
        # skip seeding
        return
    await seed_user_groups()
