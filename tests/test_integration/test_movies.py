...
#     assert response_data["imdb"] == movie_data["imdb"], "Movie imdb does not match."
#     assert response_data["price"] == movie_data["price"], "Movie price does not match."

#     for genre in movie_data["genres"]:
#         stmt = select(GenreModel).where(GenreModel.name == genre["name"])
#         result = await db_session.execute(stmt)
#         genre_obj = result.scalars().first()
#         assert genre_obj is not None, f"Genre {genre['name']} was not created."

#     for actor in movie_data["stars"]:
#         stmt = select(StarModel).where(StarModel.name == actor["name"])
#         result = await db_session.execute(stmt)
#         actor_obj = result.scalars().first()
#         assert actor_obj is not None, f"Actor {actor['name']} was not created."
        
#     for director in movie_data["directors"]:
#         stmt = select(DirectorModel).where(DirectorModel.name == director["name"])
#         result = await db_session.execute(stmt)
#         director_obj = result.scalars().first()
#         assert director_obj is not None, f"Actor {director['name']} was not created."
...
# ])
# @pytest.mark.asyncio
# async def test_post_movie_access_control(client, db_session, settings, role, expected_status, seed_database):
...
#     # NOW CREATE THE FAKE TABLE IN THIS SESSION
...
#         )
#     """))
#     await db_session.execute(text("INSERT OR IGNORE INTO certifications (id, name) VALUES (2, 'PG')"))
