...
async def test_movie_list_with_pagination(client, db_session, seed_database):
    ...
    expected_prev_page = (
        f"/movies/?page={page - 1}&per_page={per_page}&sort_by=imdb&sort_order=desc"
        if page > 1
        else None
    )
    expected_next_page = (
...
async def test_get_movie_by_id_valid(client, db_session, seed_database):
    ...
    assert (
        response_data["name"] == expected_movie.name.lower()
...
async def test_get_movie_by_id_fields_match_database(client, db_session, seed_database):
    ...
    assert (
        response_data["certification_id"] == random_movie.certification_id
...
async def test_post_movie_access_control(
...
async def test_post_movie_duplicate_error(
    ...
    response = await client.post("/moderator/movies/", json=payload, headers=headers)
    response2 = await client.post("/moderator/movies/", json=payload, headers=headers)
    assert (
...
@pytest.mark.asyncio(loop_scope="session")
async def test_delete_movie_not_found(client, db_session, jwt_manager):
    """
...
async def test_update_movie_success(client, db_session, jwt_manager, seed_database):
    ...
    response = await client.patch(
        f"/moderator/movies/{movie_id}/", json=update_data, headers=headers
    )
    assert (
        response.status_code == 200
    ), f"Expected status code 200, but got {response.status_code}"
    ...
    assert updated_movie.year == update_data["year"], "Movie year was not updated."


@pytest.mark.asyncio(loop_scope="session")
async def test_update_movie_not_found(client, db_session, jwt_manager):
    """
    Test the `/movies/{movie_id}/` endpoint with a non-existent movie ID.
    """
    non_existent_id = 99999
