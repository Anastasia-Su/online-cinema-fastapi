...
async def test_register_user_password_validation(
    client, invalid_password, expected_error
):
    """
...
@pytest.mark.asyncio
async def test_register_user_conflict(client, db_session):
    """
    ...
    response_first = await client.post("/accounts/register/", json=payload)
    assert (
    ...
    response_second = await client.post("/accounts/register/", json=payload)
    assert (
...
async def test_activate_account_success(client, db_session):
    ...
    assert user.activation_tokens, "User should have at least one activation token"
    token = user.activation_tokens[-1]
    assert token.token is not None

    activation_payload = {
        "email": registration_payload["email"],
        "token": user.activation_tokens[-1].token,
    }

    activation_response = await client.get(
        "/accounts/activate/", params=activation_payload
    )
    assert (
...
@pytest.mark.asyncio
async def test_request_password_reset_token_for_inactive_user(client, db_session):
    """
...
async def test_reset_password_success(client, db_session):
    ...
    reset_response = await client.post(
        "/accounts/reset-password/complete/", data=reset_payload
    )
    assert (
        reset_response.status_code == 200
...
async def test_reset_password_invalid_token(client, db_session):
    ...
    response = await client.post("/accounts/register/", json=registration_payload)
    ...
    response = await client.post(
        "/accounts/reset-password/complete/", data=reset_complete_payload
    )
...
async def test_refresh_access_token_success(client, db_session, jwt_manager):
    ...
    login_response = await client.post("/accounts/login/", json=login_payload)
    assert (
    ...
    refresh_response = await client.post("/accounts/refresh/", json=refresh_payload)
    assert (
...
@pytest.mark.asyncio
async def test_refresh_access_token_user_not_found(client, db_session, jwt_manager):
    """
...
@pytest.mark.asyncio
async def test_logout_user_success(client, db_session, jwt_manager):
    """
    ...
    - Call /logout endpoint with the access token.
    - Verify that the refresh token is removed from the database.
    - Verify that the response status is 204.
    """
    # 1. Create a test user
    test_user = UserModel.create(
        email="logouttest@example.com", raw_password="StrongPassword123!", group_id=2
    )
    db_session.add(test_user)
    await db_session.commit()
    await db_session.refresh(test_user)

    # 2. Create a refresh token manually
    refresh_token_str = jwt_manager.create_refresh_token({"user_id": test_user.id})
    refresh_token = RefreshTokenModel.create(
        user_id=test_user.id, token=refresh_token_str, days_valid=1
    )
    ...
    response = await client.post("/accounts/logout", headers=headers)
    assert (
        response.status_code == 204
    ), f"Expected status code 204, got {response.status_code}"

    # 5. Verify refresh token is removed
    stmt = select(RefreshTokenModel).where(RefreshTokenModel.user_id == test_user.id)
    result = await db_session.execute(stmt)
    token_record = result.scalars().first()
    assert token_record is None, "Refresh token was not deleted after logout"


@pytest.mark.asyncio
async def test_logout_user_missing_token(client):
    """
    Test logout with missing Authorization header.
...
#     """
#     Test logout with invalid or malformed token.
#     Should return 401 Unauthorized.
#     """
...
# @pytest.mark.asyncio(loop_scope="session")
# async def test_logout_user_already_revoked_token(
#     client, db_session, jwt_manager, fake_redis
# ):
#     """
#     Test logout with a token that was already revoked.
#     Should still succeed and return 204, as logout endpoint handles revocation.
#     """
#     # Create user
#     test_user = UserModel.create(
#         email="revoketest@example.com", raw_password="StrongPassword123!", group_id=2
#     )
#     db_session.add(test_user)
#     await db_session.commit()
#     await db_session.refresh(test_user)
...
#     # Simulate revoked token by adding to Redis
#     await fake_redis.set(f"revoked:{access_token}", "revoked", ex=3600)
...
@pytest.mark.asyncio
async def test_revoke_and_check_token(fake_redis):
    token = "testtoken123"
    expires_at = datetime.now(timezone.utc) + timedelta(seconds=60)

    # Initially, token is not revoked
    revoked = await is_token_revoked(token, fake_redis)
    assert revoked is False

    # Revoke token
    await revoke_token(token, expires_at, fake_redis)
...
@pytest.mark.asyncio
async def test_revoke_token_expired(fake_redis):
    token = "expiredtoken"
    expires_at = datetime.now(timezone.utc) - timedelta(seconds=1)

    # Revoke expired token should do nothing
    await revoke_token(token, expires_at, fake_redis)
    revoked = await is_token_revoked(token, fake_redis)
    assert revoked is False
...
@pytest.mark.asyncio
async def test_list_revoked_tokens(fake_redis):
    await fake_redis.set("revoked:token1", "1")
    await fake_redis.set("revoked:token2", "1")
